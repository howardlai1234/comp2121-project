...
<body>
	...
	<?php
		...
		if (!empty($_GET['productname'])) {
			...
			$productname = "Barbecued Pork Rice";
		}
		$sql = "SELECT Product.productID, Product.productName, ProductType.typeName, ProductOrigin.locationName, Product.description, Product.price, Product.ingredients, Product.imageURL, Product.stock FROM Product, ProductOrigin, ProductType WHERE Product.typeID=ProductType.typeID AND Product.originID=ProductOrigin.locationID AND Product.productName='$productname' ;";
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="heading-title text-center">
					<h2><br><?php echo $row['productName']; ?><br></h2>
					<img src="..<?php echo $row['imageURL']; ?>" class="img-fluid" alt="Image" width="320" height="240">
					<h3><?php echo $row['description']; ?></h3>
					<h2 style='color:orangered;'>$<?php echo $row['price']?></h2>
					<h3> <br>Country of Origin:<br></h3>
					<h4><?php echo $row['locationName']?></h4>
					<h3> <br>Ingredients:<br></h3>
					<h4><?php echo $row['ingredients']?></h4>
					<?php if ($row['stock'] >= 10) { ?>
					<form id="orderForm" action="" method = "post">
						...
						<button class="btn btn-common" id="submit" type="submit">Order</button>
					</form>
					<?php }else{ ?>
						<h2 style='color:orangered;'>Sorry, this porduct is currently out of stock,<br>We apologise for any inconvenience</h2>
					<?php } ?>
					<button class="btn btn-common" onclick="document.location='../menu.php'">Return to Menu</button>
				</div>
			</div>
		</div>		
	</div>
